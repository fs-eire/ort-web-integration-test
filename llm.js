...
export class LLM {
  ...
  async load(model, options) {
    ...
    const model_path =
      "https://huggingface.co/onnx-community/Qwen3-0.6B-ONNX/resolve/main";
    ...
    let externaldata = use_externaldata
      ? await fetchAndCache(model_path + "/onnx/" + "model.onnx" + "_data")
      : false;
    ...
    if (opt.executionProviders[0] === "webgpu") {
      // opt.executionProviders = [{ name: 'webgpu', enableGraphCapture: true }];
      ...
      if (options.fix) {
        opt.executionProviders[0].forceCpuNodeNames = [
          "/model/attn_mask_reformat/attn_mask_subgraph/Gather/Cast",
        ];
      }
    }
  ...
  async generate(tokens, callback, options) {
        ...
        for (const [name, t] of Object.entries(outputs)) {
          if (t.location === "cpu" && t.type != "float16") {
            ...
            options.cons_log.push(
              `"${name}": {"shape": [${
                t.dims
              }], "output-avg": ${avg}, "output-val": [${t.data.slice(
                0,
                10000000
              )}]},`
            );
    ...
    return [
      ...
      (end_time - first_token_time) / 1000,
